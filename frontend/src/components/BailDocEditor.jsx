...
import { RefreshCw, Eye, Plus, X, FileText, Sparkles, Building2, User, Download } from 'lucide-react';
...
<h1>عقد كراء</h1>
<p>رقم العقد: {{bail.numero}}</p>
<p>بين الموقعين أسفله:</p>
<p><strong>الطرف الأول (المكري):</strong></p>
<p>{{proprietaire.nom_complet}}</p>
<p><strong>الطرف الثاني (المكتري):</strong></p>
<p>{{locataire.nom_complet}}</p>
<p><strong>موضوع العقد:</strong></p>
<p>كراء {{unite.type}} الكائن بـ {{unite.adresse}}</p>
<p><strong>مدة العقد:</strong></p>
<p>من {{bail.date_debut}} إلى {{bail.date_fin}}</p>
<p><strong>السومة الكرائية:</strong></p>
<p>{{bail.montant_loyer}} درهم</p>
<p><strong>تجهيزات الوحدة:</strong></p>
<table style="width: 100%; border-collapse: collapse; border: 1px solid #000;">
  <thead>
    <tr>
      <th style="border: 1px solid #000; padding: 8px; background-color: #f0f0f0;">التجهيز</th>
      <th style="border: 1px solid #000; padding: 8px; background-color: #f0f0f0;">الحالة / ملاحظات</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
const BailDocEditor = forwardRef(({ bail }, ref) => {
  const id = bail?.id;
  const { toast } = useToast();
  const { data: tplData, isFetching } = useGetBailEditorTemplateQuery(id, { skip: !id });
  const [renderPreview, { isLoading: isRendering }] = useRenderBailPreviewMutation();

  const [model, setModel] = useState('');
  const editorInstanceRef = useRef(null);

  const initialHtml = useMemo(() => (bail?.doc_content || tplData?.template || ''), [bail, tplData]);
  const initialVars = useMemo(() => {
    const stored = bail?.doc_variables || {};
    const fresh = tplData?.variables || {};
    const merged = { ...stored, ...fresh };
    // Normalize boolean display for renouvellement_auto
    let raw = merged['bail.renouvellement_auto'];
    if (raw === undefined || raw === null) {
      raw = bail?.renouvellement_auto;
      if (raw !== undefined && raw !== null) {
        merged['bail.renouvellement_auto'] = raw;
      }
    }
    if (raw !== undefined && raw !== null) {
      const truthy = typeof raw === 'boolean' ? raw : ['1','true','oui','yes'].includes(String(raw).toLowerCase());
      merged['bail.renouvellement_auto'] = truthy ? 'Oui' : 'Non';
    }
    return merged;
  }, [bail, tplData]);
  ...
  const [templateKey, setTemplateKey] = useState(bail?.doc_template_key || tplData?.template_key || 'bail_default');
  ...
  const loadTemplate = () => {
    setModel(TEMPLATE_BAIL);
  };

  const handlePreview = async () => {
    try {
      // Ensure preview uses normalized value for renouvellement_auto
      const v = { ...vars };
      let raw = v['bail.renouvellement_auto'];
      if (raw === undefined || raw === null) {
        raw = bail?.renouvellement_auto;
        if (raw !== undefined && raw !== null) {
          v['bail.renouvellement_auto'] = raw;
        }
      }
      if (raw !== undefined && raw !== null) {
        const truthy = typeof raw === 'boolean' ? raw : ['1','true','oui','yes'].includes(String(raw).toLowerCase());
        v['bail.renouvellement_auto'] = truthy ? 'Oui' : 'Non';
      }
      const res = await renderPreview({ id, payload: { content: model, variables: v } }).unwrap();
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      // Ensure variables are normalized
      let raw = docVariables['bail.renouvellement_auto'];
      if (raw === undefined || raw === null) {
        raw = bail?.renouvellement_auto;
        if (raw !== undefined && raw !== null) {
          docVariables['bail.renouvellement_auto'] = raw;
        }
      }
      if (raw !== undefined && raw !== null) {
        const truthy = typeof raw === 'boolean' ? raw : ['1','true','oui','yes'].includes(String(raw).toLowerCase());
        docVariables['bail.renouvellement_auto'] = truthy ? 'Oui' : 'Non';
      }
      ...
      const opt = {
        margin:       [20, 12, 20, 12],
        filename:     `bail_${bail?.numero_bail || id}.pdf`,
          ...
          <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg font-semibold text-slate-800">Éditeur de Bail (Froala)</CardTitle>
              </div>
              <div className="flex items-center gap-2">
                <Tooltip>
                ...
                {id && (
                  <Tooltip>
                    ...
                    <TooltipContent>Télécharger le PDF du bail (front)</TooltipContent>
                  </Tooltip>
                )}
              </div>
            </div>
...
BailDocEditor.displayName = 'BailDocEditor';

export default BailDocEditor;
