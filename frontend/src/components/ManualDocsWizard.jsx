 ...
 */
export default function ManualDocsWizard({ open, onClose, proprietaires, defaultProprietaireId, defaultDateDebut, defaultDateFin, uniteDescription, allowAvenant, lockCoreFields = false, initialMode, defaultMandatId, unitOwners = [], ownerships = [], onBeforeSave, uniteId, editMandatId, requireAvenantFirst = false }) {
  ...
  const validateMandat = () => {
    ...
    if (!mandat.proprietaire_id && (!ownerships || ownerships.length === 0)) e.proprietaire_id = 'Choisir un propriétaire';
  ...
  const saveMandat = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // proprietaire_id: Number(mandat.proprietaire_id), // Removed in favor of proprietaires array
        ...
        proprietaires: ownerships.length > 0 ? ownerships.map(o => ({
            id: Number(o.proprietaire_id),
            part_numerateur: Number(o.part_numerateur),
            part_denominateur: Number(o.part_denominateur),
            pourcentage: (Number(o.part_numerateur) / Number(o.part_denominateur)) * 100
        })) : [
            {
                id: Number(mandat.proprietaire_id),
                part_numerateur: 1,
                part_denominateur: 1,
                pourcentage: 100
            }
      ...
      if (effectiveUpdateId) {
        ...
        res = await updateMandat({ id: effectiveUpdateId, payload: {
          ...
          proprietaires: payload.proprietaires,
