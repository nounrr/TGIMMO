...
import { RefreshCw, Eye, Plus, X, FileText, Sparkles, Building2, Download } from 'lucide-react';
...
const BailResiliationEditor = forwardRef(({ bail }, ref) => {
  const id = bail?.id;
  const { toast } = useToast();
  const { data: tplData, isFetching } = useGetBailResiliationTemplateQuery(id, { skip: !id });
  const [renderPreview, { isLoading: isRendering }] = useRenderBailResiliationPreviewMutation();

  const [model, setModel] = useState('');
  const editorInstanceRef = useRef(null);

  const initialHtml = useMemo(() => (bail?.resiliation_doc_content || tplData?.template || ''), [bail, tplData]);
  const initialVars = useMemo(() => {
    const stored = bail?.resiliation_doc_variables || {};
  ...
  useImperativeHandle(ref, () => ({
    ...
    async getContent() {
      return {
        resiliation_doc_content: model || '',
        resiliation_doc_variables: vars,
      };
    }
  }));
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const opt = {
        margin:       [20, 12, 20, 12],
        filename:     `resiliation_bail_${bail?.numero_bail || id}.pdf`,
          ...
          <CardHeader className="bg-gradient-to-r from-red-50 to-orange-50 border-b">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg font-semibold text-slate-800">Acte de Résiliation</CardTitle>
              </div>
              <div className="flex items-center gap-2">
                <Tooltip>
...
BailResiliationEditor.displayName = 'BailResiliationEditor';

export default BailResiliationEditor;