...
export default function ApprochesProprietairesShadcn() {
  ...
  const { data, isLoading } = useGetApprocheProprietairesQuery(filters);
  const [createApproche, { isLoading: saving }] = useCreateApprocheProprietaireMutation();
  const [updateApproche, { isLoading: updating }] = useUpdateApprocheProprietaireMutation();
  const [deleteApproche, { isLoading: isDeleting }] = useDeleteApprocheProprietaireMutation();
  ...
  if (typeof window !== 'undefined') {
    console.debug('[DEBUG ApprochesProprietaires] perms check', {
      userId: user?.id,
      needed: PERMS.approches_proprietaires.create,
      canCreate: can(PERMS.approches_proprietaires.create),
      permissions,
    });
  }
  
  // Charger tous les propriétaires pour le select
  ...
  const proprietaireOptions = proprietaires.map(prop => ({
    value: prop.id,
    label: prop.nom_raison || `Propriétaire #${prop.id}`,
    data: prop
  }));

  const onChange = (field, value) => setFilters(f => ({ ...f, [field]: value }));

  const proprietaireLabelById = useMemo(() => {
    const map = {};
    for (const p of proprietaires) {
      const label = p.nom_raison || [p.prenom, p.nom].filter(Boolean).join(' ') || p.raison_sociale || `Propriétaire #${p.id}`;
      map[p.id] = label;
    }
    return map;
  }, [proprietaires]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!description.trim() || !selectedProprietaire) return;
    
    try {
      if (editingId) {
        await updateApproche({ 
          id: editingId, 
          proprietaire_id: selectedProprietaire.value, 
          description 
        }).unwrap();
        setEditingId(null);
      } else {
  ...
  const handleEdit = (approche) => {
    ...
    const propOption = proprietaireOptions.find(opt => opt.value === approche.proprietaire_id);
    if (propOption) {
      setSelectedProprietaire(propOption);
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!can(PERMS.approches_proprietaires.create) && (
        <Card className="border-amber-300 bg-amber-50">
          <CardContent className="py-3 text-sm text-amber-700">
            ...
            Permission requise: <code>{PERMS.approches_proprietaires.create}</code>
          </CardContent>
        </Card>
      )}

      {/* Create/Edit Form */}
      {(can(PERMS.approches_proprietaires.create) || (editingId && can(PERMS.approches_proprietaires.update))) && (
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {selectedProprietaire && (
                    <div className="mt-2 p-2 bg-slate-50 rounded-md flex items-center gap-2 border">
                      ...
                      <div className="text-sm">
                        <div className="font-semibold">{selectedProprietaire.label}</div>
                        {selectedProprietaire.data?.email && (
                          <div className="text-muted-foreground flex items-center gap-1">
                            <span className="text-xs">{selectedProprietaire.data.email}</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="flex gap-2 pt-2">
                <Button
                  type="submit"
                  disabled={saving || updating || !selectedProprietaire || !description.trim()}
                  className="gap-2"
                >
                  {saving || updating ? (
                    <>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1 space-y-2">
              ...
              <Select
                ...
                onChange={(selected) => {
                  setFilterProprietaire(selected);
                  onChange('proprietaire_id', selected ? selected.value : '');
                }}
                options={proprietaireOptions}
                styles={customSelectStyles}
                placeholder="Rechercher un propriétaire..."
            ...
            <div className="flex items-end">
              <Button 
                ...
                onClick={() => {
                  setFilters({ proprietaire_id: '' });
                  setFilterProprietaire(null);
                }}
              >
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-slate-50 rounded-lg border border-dashed">
              ...
              <p className="text-muted-foreground">
                {filters.proprietaire_id ? 'Aucune approche pour ce propriétaire' : 'Commencez par filtrer par propriétaire'}
              </p>
            </div>
          ) : (
                    ...
                    <TableRow key={approche.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{proprietaireLabelById[approche.proprietaire_id] || `Propriétaire #${approche.proprietaire_id}`}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-2">
                          {can(PERMS.approches_proprietaires.update) && (
                            <Button
                              variant="ghost"
                              size="sm"
                          ...
                          {can(PERMS.approches_proprietaires.delete) && (
                            <Button
                              variant="ghost"
                              size="sm"
