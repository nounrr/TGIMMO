...
export default function AvenantsMandatShadcn() {
  ...
  const mandatOptions = mandats.map(m => ({ value: String(m.id), label: m.reference || `#${m.id}` }));
  ...
  const getStatusBadge = (statut) => {
    const map = {
      ...
      applique: { className: 'bg-blue-100 text-blue-700 hover:bg-blue-100 border-blue-200', label: 'Appliqué' },
      annule: { className: 'bg-red-100 text-red-700 hover:bg-red-100 border-red-200', label: 'Annulé' },
      brouillon: { className: 'bg-slate-100 text-slate-700 hover:bg-slate-100 border-slate-200', label: 'Brouillon' },
      valide: { className: 'bg-cyan-100 text-cyan-700 hover:bg-cyan-100 border-cyan-200', label: 'Validé' },
    };
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Liste des avenants créés</p>
        </div>
        <Link to="/avenants/nouveau">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                value={mandatId !== 'all' ? mandatOptions.find(o => o.value === mandatId) : null}
                onChange={(opt) => setMandatId(opt ? opt.value : 'all')}
                isClearable
                isSearchable
                placeholder="Tous les mandats"
                noOptionsMessage={() => 'Aucun mandat'}
              ...
              {isFetching ? (
                ...
                rows.map((avenant) => (
                  <TableRow key={avenant.id}>
                    <TableCell className="font-medium">{avenant.reference || `#${avenant.id}`}</TableCell>
                    <TableCell>
                      {avenant.mandat ? (
                        <div className="flex flex-col">
                          <span className="font-medium text-sm">
                            {avenant.mandat.reference || `Mandat #${avenant.mandat_id}`}
                          </span>
                        </div>
                      ) : '-'}
                    </TableCell>
                    <TableCell>{avenant.date_signature ? new Date(avenant.date_signature).toLocaleDateString() : '-'}</TableCell>
                    <TableCell>{avenant.date_effet ? new Date(avenant.date_effet).toLocaleDateString() : '-'}</TableCell>
                    <TableCell>{getStatusBadge(avenant.statut)}</TableCell>
                    <TableCell className="text-center">
                      <div className="flex justify-center gap-2">
                        <Link to={`/avenants/${avenant.id}`}>
